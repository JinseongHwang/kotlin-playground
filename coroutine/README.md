...
## runBlocking에 대한 이해
- runBlocking 함수는 일반적은 코드와 코루틴 사이 연결점 역할을 하기 위해 만들어졌기 때문에 일시 중단 지점이 아닌 곳에서도 호춣돼 코루틴을 만들어낸다.
- runBlocking 함수가 호출되면, 호출부의 스레드를 차단하고 배타적으로 사용하는 코루틴이 만들어진다. (runBlocking 이름 그대로 동작하며 코루틴을 생성한다.)
...
- [x] 그냥 runBlocking 쓰면 될거 같은데 꼭 runTest를 써야할까? ([여기](https://github.com/JinseongHwang/kotlin-playground/blob/main/coroutine/labs/runblocking-vs-runtest.md))
